...
public class TypeEncoder {
        ...
        try {
            if (arrayOfBytes || arrayOfString) {
                ...
                for (int i = 0; i < value.getValue().size(); i++) {
                    if (i == 0) {
                        offset = (long) value.getValue().size() * Type.MAX_BYTE_LENGTH;
                    } else {
                        int bytesLength =
                                arrayOfBytes
                    ...
                    result.write(
                            toBytesPadded(new BigInteger(Long.toString(offset)), Type.MAX_BYTE_LENGTH));
                }
    ...
    static <T extends Type> byte[] encodeStructsArraysOffsets(Array<T> value) {
        ...
        for (int i = 0; i < value.getValue().size(); i++) {
            if (i == 0) {
                offset = offset * Type.MAX_BYTE_LENGTH;
            } else {
                offset += tailsEncoding.get(i - 1).length;
            }
            try {
                result.write(toBytesPadded(new BigInteger(Long.toString(offset)), Type.MAX_BYTE_LENGTH));
            } catch (IOException e) {
                e.printStackTrace();
                return null;
            }
        ...
        for (int i = 0; i < value.getValue().size(); ++i) {
            ...
            if (isDynamic(type)) {
                offsetsAndStaticValues.add(
                        toBytesPadded(
                                new BigInteger(Long.toString(dynamicOffset)), Type.MAX_BYTE_LENGTH));
